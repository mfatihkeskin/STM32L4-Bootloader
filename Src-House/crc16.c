...
void CRC_init_16(void) {
	RCC->AHB1ENR |= 1U << 12; 											// enable clock for CRC
	CRC->CR = 0x00000000;												// Reset cr register
	CRC->CR |= 1U << 0;		   											// Reset calculation
	CRC->CR |= 1U << 3;		               								// sets poly size to 16 bit
	CRC->POL |= 0x1021;    												// 16 bit poly

	CRC->INIT = 0xFFFF;    												//16 bit init value
}
...
uint16_t generateCRC_16(uint16_t *byteBuffer, uint8_t length) {
    uint16_t crc = 0;
    for (uint8_t i = 0; i < length; i++) {
...
uint16_t generateCRC16(uint8_t *byteBuffer, uint8_t length) {
